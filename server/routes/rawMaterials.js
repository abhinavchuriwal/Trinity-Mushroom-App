...
router.post('/', requirePermission('manage_raw_materials'), (req, res) => {
  const { name, default_cost_per_kg_npr, carbon_pct, nitrogen_pct, notes } = req.body;
  ...
  try {
    db.prepare(
      `INSERT INTO raw_materials (name, default_cost_per_kg_npr, carbon_pct, nitrogen_pct, notes)
       VALUES (?, ?, ?, ?, ?)`
    ).run(cleanName, num(default_cost_per_kg_npr), num(carbon_pct), num(nitrogen_pct), str(notes));
  } catch (e) {
...
router.post('/:id', requirePermission('manage_raw_materials'), (req, res) => {
  const { name, default_cost_per_kg_npr, carbon_pct, nitrogen_pct, notes, active } = req.body;
  db.prepare(
    `UPDATE raw_materials
     SET name = ?, default_cost_per_kg_npr = ?, carbon_pct = ?, nitrogen_pct = ?, notes = ?, active = ?, updated_at = datetime('now')
     WHERE id = ?`
  ).run(
    (name || '').trim(),
